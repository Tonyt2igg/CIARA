...
# Store IK solutions and interpolated values
ik_solutions_rad = []
ik_solutions_deg = []
interpolated_points = []
...
def generate_linear_trajectory(start, end, num_points=NUM_INTERP_POINTS):
...
def move_to_position(target_pos, target_orient):
    ...
    if first_waypoint_reached:
        ...
        interpolated_points.append(target_pos)
...
while remaining_waypoints:
    ...
    if distance > DISTANCE_THRESHOLD:
        ...
        trajectory = generate_linear_trajectory(previous_position, lifted_pos)
        for pos in trajectory:
            move_to_position(pos.tolist(), current_orientation)

        horizontal_pos = np.array([target_position[0], target_position[1], Z_LIFT])
        trajectory = generate_linear_trajectory(lifted_pos, horizontal_pos)
        for pos in trajectory:
            move_to_position(pos.tolist(), target_orientation)
        
        trajectory = generate_linear_trajectory(horizontal_pos, target_position)
        for pos in trajectory:
            move_to_position(pos.tolist(), target_orientation)
    else:
        trajectory = generate_linear_trajectory(previous_position, target_position)
        for pos in trajectory:
            move_to_position(pos.tolist(), target_orientation)
...
if interpolated_points:
    pd.DataFrame(interpolated_points, columns=['X', 'Y', 'Z']
                ).to_csv(os.path.join(output_dir, 'interpolated_points.csv'), index=False)
