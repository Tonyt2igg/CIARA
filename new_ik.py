...
waypoints_path = r'D:\ras_sh\waypointszs.csv'
...
# Define parameters
Z_LIFT = 0.09  # Lift height
DISTANCE_THRESHOLD = 0.2  # Threshold for lifting
NUM_INTERMEDIATE_STEPS = 5  # Intermediate steps between waypoints
...
# Flag to track when the first waypoint is reached
...
def move_to_position(target_pos, target_orient, is_lift=False):
    ...
    # Only save IK solutions after reaching the first waypoint
...
while remaining_waypoints:
    ...
    # Mark the first waypoint as reached and clear any previous IK values
    if not first_waypoint_reached:
        first_waypoint_reached = True
        ik_solutions_rad = []  # Reset to remove home position IK values
        ik_solutions_deg = []
...
# Save results (After first waypoint is reached)
output_dir = r'D:\ras_sh\sav_vs'
os.makedirs(output_dir, exist_ok=True)

ik_solutions_rad = np.array(ik_solutions_rad)
ik_solutions_deg = np.array(ik_solutions_deg)

# Save only after first waypoint, skipping home position
if ik_solutions_rad.shape[0] > 0:
