...
if __name__ == '__main__':
    ...
    if opt.input_image and opt.output_image:
        ...
        # Run the CycleGAN test script for 3 styles in one pass
        os.system(f"python test.py --dataroot {temp_input_folder} --name {exp} --model test_3styles --output_nc 1 "
                  f"--no_dropout --epoch {epoch} --imagefolder {temp_output_folder} --crop_size {imgsize} "
                  f"--load_size {imgsize} --gpu_ids {gpu_id}")

        # Move the output images for 3 styles to the specified paths
        for i in range(1, 4):  # Assuming styles are saved as input_image_fake1.png, input_image_fake2.png, input_image_fake3.png
            output_image_path = os.path.join(temp_output_folder, f"input_image_fake{i}.png")
            if os.path.exists(output_image_path):
                style_output_path = opt.output_image.replace('.png', f'_fake{i}.png')
                shutil.move(output_image_path, style_output_path)
                print(f"Output for style {i} saved to {style_output_path}")
            else:
                print(f"No output image was generated for style {i}.")
